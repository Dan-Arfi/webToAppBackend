    ...
    try {
      ...
      exec(
        ...
        async (error, stdout, stderr) => {
          ...
          exec(
            `jarsigner -verbose -sigalg SHA1withRSA -digestalg SHA1 -keystore ${projectPath}/keystore.jks -storepass ${token} -keypass ${token} ${bundlePath} alias_name`,
            async (error, stdout, stderr) => {
              if (error) {
                console.error(`Error building the app: ${error.message}`);
                return res.status(500).send("Internal Server Error");
              ...
              if (logoPath) {
                ...
                db.run(
                  "UPDATE android_finals SET  apk = ?, name = ?, packageName = ?, logo = ?, versionCode = ?, bundle = ? WHERE token = ?",
                  [
                    apkData,
                    name,
                    packageName,
                  ...
                  function (err) {
                    ...
                    res
                      .status(200)
                      .send({ token: token, name: name, packageName });
                  }
                );
              } else {
                db.run(
                  "UPDATE android_finals SET  apk = ?, name = ?, packageName = ?, versionCode = ?, bundle = ? WHERE token = ?",
                  [apkData, name, packageName, versionCode, bundleData, token],
                  function (err) {
                    if (err) {
                      console.error("Error updating APK in the database:", err);
                      return res.status(500).send("Internal Server Error");
